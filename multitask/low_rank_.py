...
def low_rank(X, y, alpha, shape_u, Z=None, prior_u=None, u0=None, v0=None, rtol=1e-6, maxiter=1000, verbose=False):
    ...
    min over u and v:
         || y - X vec(u v.T) - Z w||_2 ^2 + alpha ||u - u0||_2 ^2
    ...
    for _ in range(maxiter):
        ...
        if Z is None:
            Ky = Kron_v.T.dot(X.rmatvec(y)) + alpha * prior_u
        else:
            Ky = Kron_v.T.dot(X.rmatvec(y - np.dot(Z, w0))) + alpha * prior_u
        ...
        if Z is None:
            Ky = Kron_u.T.dot(X.rmatvec(y))
        else:
            Ky = Kron_u.T.dot(X.rmatvec(y - np.dot(Z, w0)))
